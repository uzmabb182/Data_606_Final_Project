...
### When we want to compare the means of three or more samples, One Way Variance (Anova) is the most appropriate test to use.
...
We can see the drug categories on the x_axis and percent change in tumor size on the y_axis.
Infubinol drug category has one datapoint indicating the outlier. 
Infubinol and Ketapril drug category shows that the median and mean are closely alligned, and data is normally distributed, and we can ignore the outlier in Infubinol drug category.
Ketapril drug category has more variance than the Infubinol drug category.
However, Capomulin drug category shows that the mean < median in the Capomulin drug category, making it left skewed. Also, the percent change is tumor size is below zero.
So, Capomulin drug category appears to be different than Infubinol and Ketapril drug category, which is a indication that we will find the differnce between Capomulin drug category and the other two categories.
...
# H0 = mu0 = mu01 = mu02(There is no difference between the mean percent change in tumor volume for the four drug categories.)
...
We are not sure about which category mean is different, so we perform Post-hoc tests to determine that.
...
We are 95% confidence that the true mean lies between the lwr and upr bounds.
